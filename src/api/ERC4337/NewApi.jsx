...
const getUserInfo = async (userid, userpw) => {
    const {data} = await axios.get(`${BASE_API}/account/${userid}`)
...
const CreateAcc = async (data) => {
    ...
    const response = await axios.post(`${BASE_API}/account`, data) 
    return response.data
}
...
const CreateNft = async (IpfsUri, smartAcc) => {
    ...
    const {data} = await axios.post(`${BASE_API}/createnft`,_data ) 
    return data
}

const CheckZero = async () => {
    await axios.delete('${BASE_API}/checkzero')
}
